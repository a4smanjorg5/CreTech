...
export default function Host() {
  ...
  [cbt, setCBT] = useState(5),
  [lto, setLTO] = useState(10),
  ...
  useEffect(() => {
    ...
    setCBT(init => (Math.floor(sessionStorage.getItem('cbt')) || init))
    setLTO(init => (Math.floor(sessionStorage.getItem('lto')) || init))
  ...
  lockState = unlocked
  if (peer?.disconnected) peer.reconnect()
  const hostURL = hID && new URL(pathname + '/' + hID, location)
